...
class RNAConverter():
    ...
    def build_cords(self, seq, fram, angl, angl_0_radian, rtn_cmsk=False):
        ...
        for resd_name in RNA_CONSTANTS.RESD_NAMES:
            ...
            if len(idxs) == 0:
                continue
            cord[idxs], cmsk[idxs] =\
                self.__build_cord(resd_name, fram[idxs], fmsk[idxs], angl[idxs], angl_0_radian, amsk[idxs])

        return (cord, cmsk) if rtn_cmsk else (cord)

    def __build_cord(self, resd_name, fram, fmsk, angl, angl_0_radian, amsk):
        """"""
        ...
        for idx_rgrp, rgrp_name_curr in enumerate(rgrp_names_all):
            ...
            if idx_rgrp == 2:
                rot_addi, tsl_addi = calc_angl_rot_tsl(torch.tensor([[torch.cos(angl_0_radian).item(), torch.sin(angl_0_radian).item()]]))   # calculate the additional rotation and translation based on the input angle of the torsional angle of the "current" rigid group frame TODO UNDERSTAND
            else:
...
init_radian = torch.acos(angl[0][2][0])
